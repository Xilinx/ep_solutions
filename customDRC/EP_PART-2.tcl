# This DRC flags the number of hierarchical modules with the property USER_SLR_ASSIGNMENT=SLRx assigned to a specific SLR
...
set DRCINFO [list \
  ...
  {Property USER_SLR_ASSIGNMENT=SLRx on hierarchical modules} \
  {Advisory}
  ]
...
proc [lindex $DRCINFO 0] {} {
  ...
  if {[llength $cells]} {
    foreach slr [get_slrs -quiet] {
      set L [lsort -dictionary [filter $cells "USER_SLR_ASSIGNMENT == $slr"]]
      if {[llength $L]} {
        set msg "Property USER_SLR_ASSIGNMENT=$slr found on [llength $L] hierarchical modules. The first 10 modules are: [lrange $L 0 9]"
