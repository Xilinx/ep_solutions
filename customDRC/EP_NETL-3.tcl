# This DRC flags very high fanout nets (>=100K)
...
set threshold 100000 ; catch { set threshold [::drc::get_value very_high_fanout $threshold] }

set DRCINFO [list \
  [file rootname [file tail [info script]]] \
  {EmuProto_checks} \
  {Netlist Checks} \
  "Net with very high fanout (>=[expr int($threshold/1000)]K)" \
  {Warning}
  ]
...
proc [lindex $DRCINFO 0] {} {
  ...
  set threshold 100000 ; catch { set threshold [::drc::get_value very_high_fanout $threshold] }
  # DRC code
  set vhfn [get_nets -quiet -hierarchical -top_net_of_hierarchical_group -segment -filter [format { NAME =~  "*" && TYPE == "SIGNAL" && ( FLAT_PIN_COUNT >= %s) } $threshold] ] ; llength $vhfn
  if {[llength $vhfn]} {
    set L [list]
    foreach net $vhfn fpin [get_property -quiet FLAT_PIN_COUNT $vhfn] {
    ...
    foreach elm $L {
      ...
      set msg "Net %ELG has a very high fanout (fanout=$fo / driver=[get_property -quiet REF_NAME $driver]/[get_property -quiet REF_PIN_NAME $driver])"
